...
- **Agent view:** Create conversations, choose a **backend** (Cesium Agent, Cursor SDK / ACP, Codex, OpenCode, Devin, Grok Build, Claude Code, Pi, Google Antigravity over the official ACP server when installed), send messages, approve tool permissions when the provider supports it. Live updates use **`/ws/agent`**.
