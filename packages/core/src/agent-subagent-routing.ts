...
export function isGoogleAntigravitySubagentTaskToolEvent(event: SubagentToolCallEvent): boolean {
  const rawUpdate = getToolRawUpdate(event);
  if (rawUpdate?.type === "subagent.spawned") {
    return true;
  }
  const rawToolName =
    typeof rawUpdate?.toolName === "string"
      ? rawUpdate.toolName
      : typeof rawUpdate?.tool_name === "string"
        ? rawUpdate.tool_name
        : undefined;
  if (rawToolName === "invoke_subagent") {
    return true;
  }
