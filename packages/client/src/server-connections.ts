...
function notifyStateChange(): void {
  getClientPlatform().emitEvent(SERVER_CONNECTIONS_EVENT);
}
...
export function readStoredServerConnectionsState(
  ...
  try {
    ...
    const raw = store.getItem(SERVER_CONNECTIONS_STORAGE_KEY);
    if (!raw) {
      cachedState = fallback;
      store.setItem(SERVER_CONNECTIONS_STORAGE_KEY, JSON.stringify(fallback));
      return fallback;
    }
    cachedState = normalizeServerConnectionsState(JSON.parse(raw), configuredDefaultBaseUrl);
    store.setItem(SERVER_CONNECTIONS_STORAGE_KEY, JSON.stringify(cachedState));
    return cachedState;
  } catch {
    cachedState = fallback;
    return fallback;
...
export function bootstrapStoredServerConnection(
  ...
  try {
    const raw = clientKeyValueStore().getItem(SERVER_CONNECTIONS_STORAGE_KEY);
    if (raw) {
...
export function writeStoredServerConnectionsState(state: ServerConnectionsState): void {
  cachedState = state;
  try {
    clientKeyValueStore().setItem(SERVER_CONNECTIONS_STORAGE_KEY, JSON.stringify(state));
  } catch {
