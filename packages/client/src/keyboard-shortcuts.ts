...
export const SHORTCUT_COMMAND_DEFINITIONS: ShortcutCommandDefinition[] = [
  ...
  {
    id: "chat.action.cycleAgentBackend",
    label: "Chat: Cycle backend",
    section: "Chat",
    defaultBindings: ["Mod+Alt+Tab"],
    allowInEditableContexts: true,
  },
  {
...
const LEGACY_AGENT_SWITCHER_COMMAND_IDS = {
  forward: "chat.action.agentRailNextConversation",
  backward: "chat.action.agentRailPreviousConversation",
} as const;

/** Matches switcher chords, including legacy Mod+Tab bindings on rail-cycle commands. */
export function eventMatchesAgentSwitcherChord(
  event: KeyboardEvent,
  ...
  if (eventMatchesShortcutCommand(event, commandId, bindings, platform)) {
    return true;
  }
  const legacyId = LEGACY_AGENT_SWITCHER_COMMAND_IDS[direction];
  const legacyBindings = getShortcutBindingsForCommand(bindings, legacyId);
  for (const bindingStr of legacyBindings) {
    ...
    if (
      ...
      step.mod &&
      step.key === "Tab" &&
      matchesShortcutStep(event, step, platform)
    ) {
      return true;
    }
  }
...
export function eventMatchesShortcutCommand(
  ...
  const bindingList = getShortcutBindingsForCommand(bindings, commandId);
  for (const bindingStr of bindingList) {
    const parsed = parseShortcutBinding(bindingStr);
    if (!parsed || parsed.length !== 1) {
      continue;
    }
