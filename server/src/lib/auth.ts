...
async function getAuthState(): Promise<PersistedAuthState> {
  if (!authStatePromise) {
    authStatePromise = (async () => {
      const existing = await readJsonFile<PersistedAuthState | null>(
        AUTH_STATE_FILE,
        null
      );
      if (
      ...
      await writeJsonFile(AUTH_STATE_FILE, created);
      return created;
    })();
  }
...
async function createSessionRecord(input: {
  ...
  const session: AuthSessionRecord = {
    ...
    expiresAt:
      now +
      (input.remember ? config.rememberSessionTtlMs : config.sessionTtlMs),
...
export function buildUpgradeHttpResponse(result: Exclude<UpgradeAuthResult, { ok: true }>): string {
  const lines = [
