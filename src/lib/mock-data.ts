        ...
        },
        {
          ...
          children: [
            ...
            },
            {
              ...
              type: "folder",
              children: [
                { name: "IDELayout.tsx", type: "file", language: "typescript" },
              ],
            },
            {
              name: "editor",
              ...
              children: [
                ...
                { name: "EditorTabs.tsx", type: "file", language: "typescript" },
              ],
            },
            {
              name: "chat",
              type: "folder",
              children: [
                { name: "StickyChatHeader.tsx", type: "file", language: "typescript" },
              ],
            },
          ],
        },
...
const EXPLORER_DEMO: Record<
  string,
  Pick<ExplorerOpenRequest, "language" | "content" | "icon">
> = {
  "README.md": {
    language: "markdown",
    icon: "markdown",
...
Demo workspace for the OpenCursor-style IDE shell.

## Try it

- Click files in the sidebar — they open as editor tabs (demo sources).
- Split the editor and focus a pane; new files open in the focused column.
- Drag tabs between columns when split.

This README is static mock content, not read from disk.
`,
  },
  "package.json": {
    language: "json",
...
/* …trimmed demo: your real file is longer */
`,
  },
  "src/app/layout.tsx": {
    language: "typescript",
...
`,
  ...
  "src/app/page.tsx": {
    language: "typescript",
    icon: "typescript",
    content: `import { IDELayout } from "@/components/layout/IDELayout";
...
export function Button({
  ...
  return (
    <button
      type="button"
      className={\`rounded-md px-3 py-1.5 text-sm \${className}\`}
      {...rest}
    >
      {children}
    </button>
...
/** Three-pane shell: sidebar, editor, agent chat. */
...
import { useReducer, useCallback, useEffect, type DragEvent } from "react";
...
/** Demo buffer: mirrors the real EditorPanel — open from the file tree. */
export function EditorPanel() {
  ...
  // …split panes, tab DnD, overflow close — see full project file …
  return null;
}
`,
  },
...
/** Demo buffer: tab strip + split control + overflow menu (trimmed). */
export function EditorTabs() {
  ...
  return (
    <div ref={stripRef} className="flex min-h-0 items-center gap-1 overflow-x-auto hide-scrollbar-x">
      {/* …tab rows, Columns2 split 28×28, MoreVertical overflow … */}
    </div>
  );
}
`,
...
/** Demo buffer: sticky user header with transparent chrome when short enough. */
export function StickyChatHeader({
  ...
  useLayoutEffect(() => {
    ...
    if (!el) return;
    const measure = () => setAllowSticky(el.scrollHeight <= MAX_STICKY_HEIGHT_PX);
    measure();
  ...
  const stickyClass =
    enabled && allowSticky
      ? "sticky top-0 z-10 shrink-0 bg-transparent pb-[10px]"
      : "relative z-0 shrink-0 pb-[10px]";

  return (
    <div ref={ref} className={stickyClass} style={{ top: pushUpPx }}>
      {children}
    </div>
  );
}
...
function inferExplorerTabIcon(
  fileName: string,
  language?: string
): ExplorerOpenRequest["icon"] {
...
/** Build editor open payload for a file node in the demo tree (canned or stub). */
export function resolveExplorerOpenRequest(
  path: string,
  node: FileNode
): ExplorerOpenRequest {
  const preset = EXPLORER_DEMO[path];
  if (preset) {
    return {
      path,
      name: node.name,
  ...
  return {
    ...
    name: node.name,
    language,
    icon: inferExplorerTabIcon(node.name, node.language),
    content: `// Demo placeholder — "${path}"\n// Add an entry to EXPLORER_DEMO in mock-data.ts for full sample content.\n// In production, load buffer contents from your workspace API.\n\nexport {};\n`,
  };
}
...
export const editorTabs: EditorTab[] = [
  {
    ...
    icon: "terminal",
    active: true,
    content: `Last login: Sun Mar 22 09:14:02 2026 from 127.0.0.1
ben@studio:~/Projects/DEMO-PROJECT$ npm run dev
...
> next dev
 ...
 ✓ Starting...
 ✓ Ready in 1.2s
ben@studio:~/Projects/DEMO-PROJECT$ `,
  },
  {
    id: "config",
    name: "config.json",
    language: "json",
    icon: "json",
    content: `{\n  "name": "demo-project",\n  "version": "1.0.0",\n  "description": "A demo project for OpenCursor",\n  "main": "src/index.ts",\n  "scripts": {\n    "dev": "next dev",\n    "build": "next build"\n  }\n}`,
  },
  {
    id: "plan",
    name: "demo-refactor.plan.md",
    language: "markdown",
    icon: "markdown",
    content: `# Demo Refactor Plan\n\n## Overview\n\nRefactor the codebase to be more object-driven and modular.\n\n## Goals\n\n- Event-driven architecture\n- Modular component structure\n- Improved latency`,
  },
];
