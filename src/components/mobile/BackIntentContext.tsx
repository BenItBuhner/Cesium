...
type BackIntentContextValue = {
   ...
   */
  register: (priority: number, handler: () => boolean) => () => void;
...
export function BackIntentProvider({ children }: { children: ReactNode }) {
  ...
  const register = useCallback(
    (priority: number, handler: () => boolean) => {
      const id = ++nextIdRef.current;
      entriesRef.current = [...entriesRef.current, { id, priority, handler }];
      notify();
      return () => {
  ...
  const value = useMemo<BackIntentContextValue>(
    () => ({ register, handleBack, canHandleBack, subscribe }),
    [register, handleBack, canHandleBack, subscribe]
  );
...
export function useBackHandler(
  ...
  useEffect(() => {
    ...
    return ctx.register(priority, () => onBackRef.current() !== false);
  }, [ctx, active, priority]);
}
